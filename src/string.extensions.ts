interface Array<T> {
    ...
    whereDate(propName: string, value: string): Array<T>;
    where(propName: string, value: string): Array<T>;
    count(propName: string, value: string): number;
    whereContains(propName: string, value: string): Array<T>;
    first(propName: string, value: string): T;
    take(amount: number): Array<T>;
    skip(amount: number): Array<T>;
    position(propName: string, value: string): number;
    clone(): Array<T>;
    remove(amount: number): Array<T>;
    insert(amount: number, item: T): Array<T>;
    last(amount: number): Array<T>;
}
interface String {
...
String.prototype.contains = function (data: string): boolean {
...
Array.prototype.first = function (propName, value) {
    ...
    for (var i = 0; i < this.length; i++) {
        var obj = this[i];
        if (obj.hasOwnProperty(propName)) {
            //console.log(`comparing ${propName} for ${value} and ${obj[propName]}`);
